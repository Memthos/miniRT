...
bool	hit_sphere(t_minirt *rt, t_ray *ray, t_obj *obj, double min, double max, t_hit_point *p)
{
	...
	double	c = vec_squared_magnitude(oc) - obj->u_obj.sphere.diameter * obj->u_obj.sphere.diameter * 0.25;
	double	d = h * h - a *c;
	if (d < 0)
		return (false);
	double	sqrtd = sqrt(d);
	...
	t_vec3	normal = vec_scale(vec_sub(p->point, obj->u_obj.sphere.position), 1 / (obj->u_obj.sphere.diameter / 2));
