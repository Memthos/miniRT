...
/*   Updated: 2026/06/30 17:51:05 by juperrin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	const t_vec2 screen)
{
	...
	const t_vec3	vel = vec_add(
			...
			vec_add(vec_scale(cam->right, cam->move.velocity.x),
	...
	cam->aa = aa;
	cam->pos = vec_add(cam->pos, vec_scale(vel,
				cam->move.speed * (delta * 0.000001)));
	cam->viewport.delta_x = vec_scale(vx, 1.0 / screen.x);
	cam->viewport.delta_y = vec_scale(vy, 1.0 / screen.y);
	cam->viewport.nw_pos = vec_add(cam->pos, cam->dir);
	cam->viewport.nw_pos = vec_sub(cam->viewport.nw_pos, vec_scale(vx, 0.5));
	cam->viewport.nw_pos = vec_sub(cam->viewport.nw_pos, vec_scale(vy, 0.5));
	cam->viewport.nw_pixel = cam->viewport.nw_pos;
	return (cam);
}
